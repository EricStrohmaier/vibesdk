        ...
        try {
                for (const entry of readdirSync(dir, { withFileTypes: true })) {
                        ...
                        if (entry.isDirectory()) {
                                result.push(
                                        ...listFilesRecursive(full, baseDir),
                                );
                        } else {
...
function createInstance(body: {
        ...
        if (body.envVars && Object.keys(body.envVars).length > 0) {
                const content = Object.entries(body.envVars)
                        .map(([k, v]) => `${k}=${v}`)
                        .join('\n');
        ...
        if (existsSync(join(workDir, 'package.json'))) {
                ...
                execInDir(
                        'bun install --no-progress 2>&1 || npm install 2>&1',
                        workDir,
                        120_000,
                );
        }
        ...
        console.log(
                `[sandbox] Starting ${portedCommand} in ${workDir} on port ${port}`,
        );
        ...
        proc.stdout?.on('data', (data: Buffer) => {
                ...
                inst.stdout += text;
                if (inst.stdout.length > 50_000)
                        inst.stdout = inst.stdout.slice(-40_000);
        });

        proc.stderr?.on('data', (data: Buffer) => {
                const text = data.toString();
                inst.stderr += text;
                if (inst.stderr.length > 50_000)
                        inst.stderr = inst.stderr.slice(-40_000);
                if (
        ...
        proc.on('exit', (code) => {
                console.log(
                        `[sandbox] Process for ${id} exited with code ${code}`,
                );
                inst.process = null;
        });
        ...
        async fetch(req) {
                ...
                try {
                        ...
                        if (method === 'GET' && path === '/instances') {
                                const list = Array.from(instances.values()).map(
                                        (inst) => ({
                                                runId: inst.id,
                        ...
                        if (instanceMatch) {
                                ...
                                if (method === 'GET' && subPath === '/files') {
                                        const filePathsParam =
                                                url.searchParams.get(
                                                        'filePaths',
                                                );
                                        const paths = filePathsParam
                                                ? JSON.parse(
                                                          decodeURIComponent(
                                                                  filePathsParam,
                                                          ),
                                ...
                                if (method === 'DELETE' && subPath === '') {
                                        ...
                                        return json({
                                                success: true,
                                                message: `Instance ${instanceId} shut down`,
                                        });
                                }
                        }
