                ...
                async function init() {
                        ...
                        logger.debug('Received chunk from server:', obj);
                                                ...
                                                if (obj.websocketUrl) {
                                                        ...
                                                        // the local Vite proxy so auth cookies and the proxy
                                                        // work correctly (avoids cross-domain cookie issues).
                                                        try {
                                                                const parsed = new URL(obj.websocketUrl);
                                                                const localProto = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
                                                                parsed.protocol = localProto;
                                                                parsed.host = window.location.host;
                                                                result.websocketUrl = parsed.toString();
                                                        } catch {
                                                                result.websocketUrl = obj.websocketUrl;
                                                        }
                                                        logger.debug('📡 Received WebSocket URL from server (rewritten for proxy):', result.websocketUrl)
                                                }
                                                if (obj.behaviorType) {
