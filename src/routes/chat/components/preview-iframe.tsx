...
export const PreviewIframe = forwardRef<HTMLIFrameElement, PreviewIframeProps>(
        ...
        const postLoadTimeoutRef = useRef<NodeJS.Timeout | null>(null);
                 ...
                 */
                const testAvailability = useCallback(async (_url: string): Promise<'sandbox' | 'dispatcher' | null> => {
                        // The server already confirmed the container is up via deployment_completed.
                        // Skip the HTTP pre-check — the Replit devtools extension intercepts
                        // window.fetch and blocks cross-origin requests regardless of CORS mode,
                        // causing false "not available" retries. Load the iframe directly and let
                        // its own onLoad/onError events handle actual failures.
                        return 'sandbox';
                }, []);
